...
module uart_regs
   #(parameter DATA_WIDTH=8,
     ...
     // Magic address that triggers a stream read.  Setting STREAM_ADDR to
     // any value outside [0..NUM_REGS-1] effectively disables the feature.
    ...
    (input  logic i_clk,
     ...
     // Hardware-driven status values that overlay reads at
     // [STATUS_BASE .. STATUS_BASE+STATUS_NUM-1].  Ignored if STATUS_NUM=0.
    ...
    ******/
    ...
    // Combinational "what gets captured at w_rd"
    ...
    * tx arbitration
    *************/
    // In the STREAM state the TX FIFO's write port is driven by the
    // external byte stream (i_stream_byte / i_stream_valid).  Otherwise
    // it is driven by the normal 32-bit read shift path.
    ...
    *****/
    ...
    always_comb begin
        ...
        case (r_state)
            ...
            STREAM: begin
                // External module pushes bytes into the TX FIFO via
                // i_stream_byte / i_stream_valid while we sit here.
                // We leave as soon as i_stream_done pulses.
                if (i_stream_done)
                    w_next_state = IDLE;
            end
